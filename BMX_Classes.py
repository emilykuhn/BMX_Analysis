...
#         self.velocity=np.array(drone_data.vel)
    ...
    def Plot_Drone_Coordinates(self,t_bounds=[0,-1]):
        ...
        ax5.set_title('Yaw vs Time')
        ax6.set_title('Roll/Pitch vs Time')
        ...
        yqtys=[self.longitude,self.latitude,self.longitude,self.altitude,self.yaw, self.roll]
        xtags=['Latitude, [$deg$]','Drone Index','Drone Index','Drone Index','Drone Index','Drone Index']
        ytags=['Longitude, [$deg$]','Latitude, [$deg$]','Longitude, [$deg$]','Altitude, [$m$]','Yaw [$deg$]', 'Roll [$deg$]']
        for i,ax in enumerate([ax1,ax2,ax3,ax4,ax5,ax6]):
            ...
            ax.grid()
            ax.legend()
        ax6.plot(self.t_index, self.pitch, '.')
        tight_layout()
  
    def Plot_Angular_Coordinates(self,t_bounds=[0,-1]):
        fig=figure(figsize=(10,4.5))
        ax1=fig.add_subplot(1, 2, 1)
        ax1.plot(self.t_index[:],(180/np.pi)*self.coords_rpt_prime[:,1],'.')
        ax1.plot(self.t_index[t_bounds[0]:t_bounds[1]],(180/np.pi)*self.coords_rpt_prime[t_bounds[0]:t_bounds[1],1],'.')
        ax1.set_xlabel('time index')
        ax1.set_ylabel(r'$\phi, [deg]$')
        ax2=fig.add_subplot(1, 2, 2)
        ax2.plot(self.t_index[:],(180/np.pi)*self.coords_rpt_prime[:,2],'.')
        ax2.plot(self.t_index[t_bounds[0]:t_bounds[1]],(180/np.pi)*self.coords_rpt_prime[t_bounds[0]:t_bounds[1],2],'.')
    ...
    def Plot_3d(self,t_bounds=[0,-1]):
        ...
        for i,coordset in enumerate([self.coords_xyz,self.coords_xyz_prime]):
...
def droneDataPairing(drone_dir, FLY, bmx_dir, BMXFILE, pol):
...
class Do_Everything_For_Data:
    def __init__(self, drone_dir, FLY, bmx_dir, BMXFILE):
        ...
        self.fulldict = droneDataPairing(drone_dir, FLY, bmx_dir, BMXFILE, pol)    
...
###NOW: combine 2 files
###make dictionary with full parameter info
#for getting rid of box on slice lots, could do a "if label == true" thing
...
#         t_drone2 = pd.to_datetime(drn_dat2.t_arr_timestamp)+pd.Timedelta(seconds=time_offset2_s)
        ...
        return {"FLY" : FLY1, "BMXdir" :  BMXFILE1, "autos" : autos, "drone_time" : t_drone, "tel_time" : t_tel, "x" : x, "y" : y, "z" : z, "thetaX" : thetaX, "thetaY" : thetaY, "thetaX_beam" : thetaX_beam, "thetaY_beam" : thetaY_beam,"r": rprime, "yaw": yaw, "phi":phi}
